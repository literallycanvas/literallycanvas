...
(function() {
    ...
    LC.LinePathShape = function() {
        ...
        LinePathShape.prototype.addPoint = function(x, y) {
            var distance, newPoint;
            newPoint = this.tool.makePoint(x, y);
            if (this.points.length) {
                distance = LC.len(LC.diff(_.last(this.points), newPoint));
            }
            if (distance) {
                newPoint.size = newPoint.size + Math.sqrt(distance);
            }
            this.points.push(newPoint);
            if (!this.smoothedPoints || this.points.length < this.tail) {
                return this.smoothedPoints = LC.bspline(this.points, this.order);
            } else {
                return this.smoothedPoints = this.smoothedPoints.concat(_.last(LC.bspline(_.last(this.points, this.tail), this.order), Math.pow(2, this.order)));
            }
        };
        LinePathShape.prototype.draw = function(ctx) {
            ...
            poly = LC.toPoly(this.smoothedPoints);
...
(function() {
    ...
    LC.Pencil = function(_super) {
        ...
        Pencil.prototype.optionsContents = function() {
            ...
            $el.filter("input").change(function(e) {
                _this.strokeWidth = $(e.currentTarget).val();
                return $el.filter(".brush-width-val").html("(" + _this.strokeWidth + " px)");
            });
            return $el;
        };
